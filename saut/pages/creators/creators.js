...
function getAvatarUrl(avatarData) {
  if (!avatarData || !avatarData.id || !avatarData.extension) {
    return PLACEHOLDER_AVATAR;
  }
  
  // Преобразуем число расширения в строку
  const extMap = {
    108: 'webp', // webp
    35: 'png',   // png
    21: 'gif',   // gif
  ...
  const ext = extMap[avatarData.extension] || 'png';
  
  // Используем ПК-User-Agent и правильные заголовки
  return `/api/avatar?id=${avatarData.id}&ext=${ext}&ua=desktop`;
}
...
async function loadTeam() {
    ...
    try {
        ...
        creatorsData = TARGET_LOGINS.map(login => {
            ...
            const avatarUrl = user.avatarOrSkin 
              ? getAvatarUrl(user.avatarOrSkin) 
              : PLACEHOLDER_AVATAR;
...
function renderCreator(index) {
    ...
    // Рендерим содержимое БЕЗ timestamp
    container.innerHTML = `
...
function updateAvatars() {
    document.querySelectorAll('[data-original-src]').forEach(img => {
        ...
        if (originalSrc && originalSrc !== PLACEHOLDER_AVATAR) {
            // Добавляем timestamp ТОЛЬКО при обновлении
