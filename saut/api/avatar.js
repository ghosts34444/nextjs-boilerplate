...
export default async (req, res) => {
  ...
  // ВСЕГДА используем .webp (сервер Лолилэнда игнорирует расширение)
  ...
  try {
    ...
    // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Используем loliland.net для API
    const response = await fetch(
      `https://loliland.net/apiv2/user/avatar/medium/${id}.${extension}`,
      {
        headers: {
